...
vi.mock('@/services/insights.service', () => ({ getSpendingTrend: vi.fn() }))
...
describe('GET /api/households/[id]/insights/trend', () => {
  beforeEach(() => vi.clearAllMocks())

  it('returns trend for member', async () => {
    vi.mocked(auth).mockResolvedValue(session as any)
    vi.mocked(getMemberRole).mockResolvedValue('member')
    vi.mocked(getSpendingTrend).mockResolvedValue([{ month: '2026-03', total: 500 }] as any)

    const req = new Request('http://localhost/api/households/hh-1/insights/trend?months=6')
