...
vi.mock('@/services/insights.service', () => ({ getMemberBreakdown: vi.fn() }))
...
describe('GET /api/households/[id]/insights/by-member', () => {
  beforeEach(() => vi.clearAllMocks())

  it('returns member breakdown for member', async () => {
    vi.mocked(auth).mockResolvedValue(session as any)
    vi.mocked(getMemberRole).mockResolvedValue('member')
    vi.mocked(getMemberBreakdown).mockResolvedValue([{ userId: 'u1', amount: 200 }] as any)

    const req = new Request('http://localhost/api/households/hh-1/insights/by-member?month=3&year=2026')
